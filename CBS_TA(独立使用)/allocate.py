...
import numpy as np
from CBS_TA.a_star_single import Astar
import copy

'''---------------------------------------'''

class Allocate():
    def __init__(self, agent_dict,task_dict,dimension,obstacles):
        # 初始化参数
        ...
        self.agent_dict={}
        self.task_dict={}
        self.transform() # 经过数据结构转换agent_dict={0: (1, 0)},task_dict={0: [(10, 0), (10, 1)]}
        '''-------------------------------------'''
        # 创建视图
        self.scale=16 # 格子间距
        self.canvas=np.ones((self.dimension[0]//3,self.dimension[1]//3,3), np.uint8) * 255
        # self.canvas = cv2.namedWindow('window_name', cv2.WINDOW_FREERATIO)
        self.frames = []  # 窗口画面
    ...
    # 绘制图
    def draw_map(self,solution,static):
        scale=self.scale # 格子间距
        font_scale = 1
        font_size = 0.5
        color = (255, 255, 255)
        ...
        if static:
            ...
            plt.axis('off')
            plt.savefig('MAPF.pdf',format='pdf',dpi=300,bbox_inches='tight',pad_inches=0)
    ...
    def start_Hamiltonian(self):
        # 生成哈密顿环图
        self.Hamiltonian(self.agent_dict, self.task_dict, self.result_list)
    ...
    def allocating_task(self):
        st1 = time.time()
        self.result_list2, min_cost2 = self.aaa3()
        tt1 = time.time()
        a=self.result_list2
        ...
        print("最优分配序列：{0}".format(pairs))

        # pairs=[(0,1),(1,0,3),(2,2)] # 手动输入进行测验
        return pairs
