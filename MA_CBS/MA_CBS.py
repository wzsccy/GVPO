...
import heapq
import random
from MA_CBS.single_agent_planner import compute_heuristics, a_star,a_star_eight, get_location, get_sum_of_cost, EPEa_star,get_sum_of_cost_eight
    ...
    def find_solution(self,dl,start_time):
        ...
        while len(self.open_list) != 0:
            ...
            if (should_merge(agent_1, agent_2 , curr['matrix'], curr['list_meta'], self.B)):
                ...
                meta_paths = EPEa_star(dl,start_time,self.my_map, self.starts, self.goals, self.heuristics,
            ...
            for constraint in constraints:
                ...
                if len(constraint['meta-agent']) > 0:   ##If it is meta agent then run EPEA*
                    ...
                    path = EPEa_star(dl,start_time,self.my_map, self.starts, self.goals, self.heuristics,
...
class MACBSSolver_eight(object):
    """The high-level search of CBS."""
    ...
    def find_solution(self,dl,start_time):
        ...
        ########### FINAL
        conflict_matrix = [[0 for x in range(self.num_of_agents)] for y in
                           range(self.num_of_agents)]  # Create a matrix to keep track agent pairs conflicts
        ...
        root = {'cost': 0,
                ...
                'list_meta': []}
        for i in range(self.num_of_agents):  # Find initial path for each agent
            path = a_star_eight(self.start_time,self.my_map, self.starts[i], self.goals[i], self.heuristics[i],
                          i, root['constraints'])
            if path == None: # 超出时间
                return 'over_time'
            # if path is None:
        ...
        root['cost'] = get_sum_of_cost_eight(root['paths'])
        root['collisions'] = detect_collisions(root['paths'])
        self.push_node(root)
            ...
            if (should_merge(agent_1, agent_2, curr['matrix'], curr['list_meta'], self.B)):
                ...
                if meta_paths != None:
                    ...
                    for agent in meta_agent:
                        curr['paths'][agent] = meta_paths[index] + []
                        index += 1
                    curr['cost'] = get_sum_of_cost_eight(curr['paths'])
                    curr['collisions'] = detect_collisions(curr['paths'])
                    # print(meta_agent)
                    # print(curr['collisions'])
                    if upper_bound >= curr['cost']:  # len(child['paths'][agent]):
                ...
                if len(constraint['meta-agent']) > 0:  ##If it is meta agent then run EPEA*
                    ...
                    if meta_paths != None:
                        ...
                        for agent in constraint['meta-agent']:
                            ...
                            index += 1
                        child['collisions'] = detect_collisions(child['paths'])
                        child['cost'] = get_sum_of_cost_eight(child['paths'])
                        if upper_bound >= child['cost']:  # len(child['paths'][agent]):
                            self.push_node(child)
