...
from SIPP.util.draw import Draw
from SIPP.multiagent import MAPF
from SIPP.entity.Map import Map, TimestampSearchMap
from SIPP.entity.Task import Task
from SIPP.search_functions.AStar import AStar
from SIPP.util.movingai_parser import *
from random import sample
from SIPP.entity.Agent import Agent
...
def run_scens(map_filename, scen_filename, cnt_agents, search_function, stats, cnt_runs=10, draw_path=None, mapf_tl=10):
    ...
    for _ in tqdm(range(cnt_runs)):
        agents = list(map(lambda x: Agent(x[1].start_y, x[1].start_x, x[1].goal_y, x[1].goal_x, x[0]),
                          enumerate(sample(tasks, k=cnt_agents))))
