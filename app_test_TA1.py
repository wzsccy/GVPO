...
def _build_planner(args):
    planner_key = args.planner.lower()
    planner_kwargs = {}
    if planner_key == "lacam":
        planner_kwargs["flg_star"] = args.flg_star
        planner_kwargs["verbose"] = args.verbose
    elif planner_key == "lns2":
        planner_kwargs["num_neighbours"] = args.planner_neighbours
    return planner_key, PlannerFactory.create(planner_key, **planner_kwargs)
...
def _run_planner(
    ...
    request = PlannerRequest(
        grid=grid,
        starts=copy.deepcopy(starts_cfg),
        goals=copy.deepcopy(goals_cfg),
    ...
    if result.solution is None:
        return None, status or "failed", result.cost
    return result.solution, None, result.cost or 0.0
    ...
    for map in map_list:
        ...
        for N in N_list:
            ...
            for ins_num in range(1, 6):
                ...
                planner_key, planner = _build_planner(args)
                print(f"[Planner] Using backend: {planner.name}")
                ...
                for aaa in [1,2]:
                    ...
                    if aaa == 1:
                        ...
                        for i in range(0, len(AT_2[max_agent_2]) - 1):
                            ...
                            solution, status, cost = _run_planner(
                                planner,
                                grid,
                                start_Config1,
                                goal_Config2,
                        ...
                        if flag == False:
                            ...
                            TaskA_time, Total_time, Total_cost = (round(TA_end_time - TA_start_time, 2),
                                                                  ...
                                                                  round(cout_total_cost, 2))
                            if failure_status:
                                print(f"[Planner] Terminated early with status: {failure_status}")
