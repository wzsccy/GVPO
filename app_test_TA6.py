...
if __name__ == "__main__":
    # map_list = ["empty-32-32.map","empty-48-48.map","random-64-64-10.map","den312d.map",]
    # map_list = ["ost003d.map","random-64-64-20.map","room-32-32-4.map"]
    map_list = ["den312d.map"]
    # map_list = ["room-64-64-16.map","warehouse-20-40-10-2-2.map"]
    for map in map_list:
        set_map = map
        Num_T = 100
        dl_la = 10
        ...
        for N in N_list:
            ...
            for ins_num in range(1, 2):
                ...
                pairs_2, C_2 = allocate.allocating_task('TA5')
            ...
            "绘图"
            if total_sol != {}:
                copy_total_sol = allocate.transform_for_draw(pairs_2, total_sol, agent_dict, task_dict)  # 制作绘图数据
                # allocate.start_Hamiltonian()          # 展示哈密顿环
                allocate.draw_map(copy_total_sol, 0, map)  # 动态MAPD
            "结束提示"
