...
class PIBT:
    ...
    def run(self, max_timestep: int = 1500) -> Configs:
        # define priorities
        start=time.time()
        ...
        configs = Configs()
        configs.append(self.starts)

        # temp_cofig.positions.clear()
        while time.time()-start <= max_timestep:
            # obtain new configuration
            Q,cost = self.step(configs[-1], priorities)
            temp_cofig = Config()
            temp_cofig.positions= Q
            configs.append(temp_cofig)
